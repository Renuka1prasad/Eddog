...
pkg_hyperdog_gazebo = 'eddog_gazebo_sim'
robot_description_subpath = 'urdf/eddog.urdf.xacro'
xacro_file = os.path.join(get_package_share_directory(pkg_eddog_description),robot_description_subpath)
...
pkg_share = FindPackageShare(package='eddog_description').find('eddog_description')
...
def generate_launch_description():
    ...
    launch_joy_cmd_executor = ExecuteProcess(
        cmd=['ros2', 'run', 'eddog_control', 'ctrl_cmd_executor'],
        output='screen'
    )

    hyperdog_joint_controller = Node(
        package='eddog_control',
        executable='eddog_joint_controller_node',
        output='screen'
    )
